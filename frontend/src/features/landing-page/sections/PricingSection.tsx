...
export const PricingSection = ({
  ...
  const handlePlanAction = (planId: string) => {
    if (isAuthenticated && role === "pharmacy_manager") {
      ...
      <Container maxWidth="xl">
        <Box sx={{ textAlign: "center", mb: 8 }}>
          ...
          <Typography
            variant="body2"
            color="text.secondary"
            sx={{ mt: 1.5, maxWidth: 720, mx: "auto" }}
          >
            Plan payments now create a secure billing record. Pharmacy managers
            submit a payment reference, and admins approve it before access is
            activated.
          </Typography>
        </Box>
        ...
        <Grid container spacing={4} sx={{ justifyContent: "center" }}>
          {PRICING_PLANS.map((plan, index) => {
            const isEnterprise = plan.id === "enterprise_chain";
            return (
